...
- 修改文件:取决于选定方案;A 涉及 `build.sh` / `.github/workflows/` / 签名证书;B 涉及 `UsageService.swift` + 新 Keychain 写入逻辑 + v0.5.1 spec;C 仅文档。
- 风险点:A 触碰 Apple Developer 凭证;B 触碰凭证存储链路并反转已 accepted 的 spec 决策。
- 测试计划:待方案确定后补。
