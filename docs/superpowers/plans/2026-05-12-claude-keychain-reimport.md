...
|---|---|
| `macos/Sources/ClaudeUsageBar/ClaudeCLICredentialsStrategy.swift` (modify) | `loadCredentials()` → `loadCredentials(allowInteraction: Bool = true)`; when `!allowInteraction`, add `kSecUseAuthenticationUI: kSecUseAuthenticationUIFail` to the query |
| `macos/Sources/ClaudeUsageBar/UsageService.swift` (modify) | add `internal var cliKeychainLoader: () async -> StoredCredentials?` (default → real strategy with `allowInteraction: false`); `expireSession()` → `async`, entry calls new `attemptCLIKeychainRecovery()`; 4 call sites → `await expireSession()` |
| `macos/Tests/ClaudeUsageBarTests/ClaudeKeychainReimportTests.swift` (new) | the new behaviour: recover / hard-expire-when-empty / no-loop-same-token / hard-expire-when-expired / no-recovery-multi-account / normal-refresh-unaffected |
| `macos/Tests/ClaudeUsageBarTests/UsageServiceTests.swift` (modify) | the two hard-expire tests (`testFetchUsageSignsOutWhenRefreshFails`, `testExpiredTokenWithPermanentRefreshFailureSignsOut`) get `service.cliKeychainLoader = { nil }` so they still assert hard-expire after the async change |
...
**Files:**
- Modify: `macos/Sources/ClaudeUsageBar/ClaudeCLICredentialsStrategy.swift`
- Test: `macos/Tests/ClaudeUsageBarTests/ClaudeCLICredentialsStrategyTests.swift` (add one signature/compile test; behaviour is hard to unit-test without a real Keychain — keep it minimal)
...
```bash
git add macos/Sources/ClaudeUsageBar/ClaudeCLICredentialsStrategy.swift macos/Tests/ClaudeUsageBarTests/ClaudeCLICredentialsStrategyTests.swift
...
- Modify: `macos/Tests/ClaudeUsageBarTests/UsageServiceTests.swift` (two hard-expire tests)
- Test: `macos/Tests/ClaudeUsageBarTests/ClaudeKeychainReimportTests.swift` (new)
...
Run: `grep -rnE 'accessToken|refreshToken' macos/Sources/ClaudeUsageBar/UsageService.swift macos/Sources/ClaudeUsageBar/ClaudeCLICredentialsStrategy.swift | grep -E 'NSLog|print|os_log|Logger|description|"\\\(' || echo "OK: no credential interpolation in logs/errors"`
...
```bash
git add macos/Sources/ClaudeUsageBar/UsageService.swift macos/Tests/ClaudeUsageBarTests/ClaudeKeychainReimportTests.swift macos/Tests/ClaudeUsageBarTests/UsageServiceTests.swift
