...
| File | Responsibility |
|---|---|
| `macos/Sources/UsageBar/CodexCredentials.swift` (new) | `CodexCredentials` struct + `CodexCredentialStore.load(environment:)` — locate & parse auth.json; never write it |
| `macos/Sources/UsageBar/CodexUsageModel.swift` (new) | `CodexUsageResponse: Decodable` (wire shape) + `CodexPlan` + `CodexRateWindow` + `CodexCredits` + `normalizedWindows()` + `asProviderSnapshot()` |
| `macos/Sources/UsageBar/CodexUsageClient.swift` (new) | `CodexUsageClient.fetchUsage(credentials:session:)` — `GET wham/usage`; errors carry no credentials/body |
| `macos/Sources/UsageBar/CodexProvider.swift` (new) | `@MainActor final class CodexProvider: UsageProvider` — owns `ProviderRuntime`, orchestrates load→fetch→map in `refreshNow()` |
| `macos/Sources/UsageBar/ProviderUsageSnapshot.swift` (modify) | `CreditLine` += `remainingAmount: Double?`, `isUnlimited: Bool = false` |
| `macos/Sources/UsageBar/ProviderUsageSection.swift` (modify) | `CreditLineRow` += unlimited / remaining branches + non-hardcoded title |
| `macos/Sources/UsageBar/ProviderTabBar.swift` (modify) | `ProviderUnconfiguredView` Codex-specific copy ("运行 `codex` 登录") |
| `macos/Sources/UsageBar/PopoverView.swift` (modify) | extract `ProviderUsageArea` subview (`@ObservedObject runtime`); configured/unconfigured decided from `runtime.isConfigured` |
| `macos/Sources/UsageBar/ProviderCoordinator.swift` (modify) | `primaryEligibleIDs`; constructor + setter reject non-eligible `primaryProviderID` |
| `macos/Sources/UsageBar/SettingsView.swift` (modify) | Primary Provider Picker data source → `primaryEligibleIDs` |
| `macos/Sources/UsageBar/UsageBarApp.swift` (modify) | `ProviderCoordinator(claude: UsageService(), additionalProviders: [CodexProvider()])` |
| `macos/Tests/UsageBarTests/CodexProviderTests.swift` (new) | credential parsing, response decode + mapping, `refreshNow()` behaviour, SC7 redaction |
...
**Files:**
- Create: `macos/Sources/UsageBar/CodexCredentials.swift`
- Test: `macos/Tests/UsageBarTests/CodexProviderTests.swift` (new file in this task)

- [ ] **Step 1: Write the failing tests**

Create `macos/Tests/UsageBarTests/CodexProviderTests.swift`:
...
```bash
git add macos/Sources/UsageBar/CodexCredentials.swift macos/Tests/UsageBarTests/CodexProviderTests.swift
...
**Files:**
- Modify: `macos/Sources/UsageBar/ProviderUsageSnapshot.swift`
...
**Files:**
- Create: `macos/Sources/UsageBar/CodexUsageModel.swift`
- Test: append to `macos/Tests/UsageBarTests/CodexProviderTests.swift`
...
```bash
git add macos/Sources/UsageBar/CodexUsageModel.swift macos/Tests/UsageBarTests/CodexProviderTests.swift
...
**Files:**
- Create: `macos/Sources/UsageBar/CodexUsageClient.swift`
...
```bash
git add macos/Sources/UsageBar/CodexUsageClient.swift macos/Tests/UsageBarTests/CodexProviderTests.swift
...
**Files:**
- Create: `macos/Sources/UsageBar/CodexProvider.swift`
...
```bash
git add macos/Sources/UsageBar/CodexProvider.swift macos/Tests/UsageBarTests/CodexProviderTests.swift
...
**Files:**
- Modify: `macos/Sources/UsageBar/ProviderUsageSection.swift`
...
```bash
git add macos/Sources/UsageBar/ProviderUsageSection.swift macos/Sources/UsageBar/PopoverView.swift macos/Sources/UsageBar/ProviderTabBar.swift
...
```bash
git add macos/Sources/UsageBar/ProviderCoordinator.swift macos/Sources/UsageBar/SettingsView.swift macos/Sources/UsageBar/UsageBarApp.swift macos/Tests/UsageBarTests/ProviderAbstractionTests.swift
...
Run:
```bash
grep -rnE 'accessToken|refreshToken|idToken|accountId' macos/Sources/UsageBar/Codex*.swift | grep -E 'print|os_log|Logger|description|"\\\(' || echo "OK: no credential interpolation found"
```
