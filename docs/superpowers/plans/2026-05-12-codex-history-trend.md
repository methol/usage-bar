...
- `macos/Sources/UsageBar/UsageHistoryService.swift` — 改 `init` 收 `filename`/`directory`，文件路径由 `static historyFileURL` 变实例 `let fileURL`/`backupURL`。
- `macos/Sources/UsageBar/UsageProvider.swift` — 仅改 `supportsBackgroundPolling` 的文档注释。
- `macos/Sources/UsageBar/ProviderCoordinator.swift` — 仅改 `primaryEligibleIDs` 的文档注释。
- `macos/Sources/UsageBar/CodexProvider.swift` — 加 `let history`、`init(history:)` + `loadHistory()`、`recordHistorySample(from:)`、`startPolling()` + `pollCancellable`。
- `macos/Sources/UsageBar/UsageChartView.swift` — `UsageChartSectionView` / `UsageChartContentView` 加 `primaryLabel`/`secondaryLabel`。
- `macos/Sources/UsageBar/PopoverView.swift` — 新增 `ProviderHistorySection`；`ProviderUsageArea` 可选挂它；`providerArea` Codex 分支传 history。
- `macos/Sources/UsageBar/UsageBarApp.swift` — `.task` 里 `(coordinator.provider(.codex) as? CodexProvider)?.startPolling()`。
- `macos/Tests/UsageBarTests/UsageHistoryServiceTests.swift` — 新建。
- `macos/Tests/UsageBarTests/CodexProviderTests.swift` — 追加用例。
...
**Files:**
- Modify: `macos/Sources/UsageBar/UsageHistoryService.swift`
- Test: `macos/Tests/UsageBarTests/UsageHistoryServiceTests.swift` (create)
...
git add macos/Sources/UsageBar/UsageHistoryService.swift macos/Tests/UsageBarTests/UsageHistoryServiceTests.swift
...
**Files:**
- Modify: `macos/Sources/UsageBar/CodexProvider.swift`
- Test: `macos/Tests/UsageBarTests/CodexProviderTests.swift` (append)
...
final class CodexProvider: UsageProvider {
    ...
    /// 起 5 分钟的轻量后台采样（幂等）；调用即先拉一次。装配处（`UsageBarApp`）显式调用。
    func startPolling() {
...
**Files:**
- Modify: `macos/Sources/UsageBar/UsageProvider.swift`（注释）
- Modify: `macos/Sources/UsageBar/ProviderCoordinator.swift`（注释）
...
- [x] **Step 3: `UsageBarApp.swift` —— `.task` 末尾起 Codex 采样**
...
```bash
cd /Users/methol/data/code-methol/usage-bar
git add macos/Sources/UsageBar/UsageProvider.swift macos/Sources/UsageBar/ProviderCoordinator.swift macos/Sources/UsageBar/UsageBarApp.swift
...
Run: `cd /Users/methol/data/code-methol/usage-bar && make release-artifacts && bash macos/scripts/verify-release.sh macos/UsageBar.zip`
