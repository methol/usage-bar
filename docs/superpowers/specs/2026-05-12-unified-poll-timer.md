...
spec_criteria:
  ...
  - id: SC2
    ...
    evidence: null
  - id: SC3
    criterion: "**`ProviderCoordinator` 的后台 timer 现在覆盖所有 enabled provider（含 Claude）**：`onBackgroundTick()` 改成遍历 `availableIDs` 里**全部** provider（不再 `where id != .claude`）：对每个 provider `p`，若 `let due = p.nextEligibleRefresh, due > Date()` → **跳过这一 tick**（还在 backoff 窗口里）；否则 `Task { await p.refreshNow() }` + `p.onPollTick?()`（`onPollTick` 现在是协议成员，不需 `as? CodexProvider` 向下转型了）。`refreshAllEnabledOnOpen()` 同理简化（不再特判 Claude 的 snapshot==nil —— 改成：对 `availableIDs` 里每个 provider，若 `nextEligibleRefresh` 在未来则跳过，否则 `await refreshNow()`；这样 Claude 也参与，且首屏空时（snapshot==nil、不在 backoff）会被拉一次）—— 删 `shouldRefreshClaudeOnOpen`（或保留作 deprecated alias，实施挑）。`startBackgroundPolling(...)` 不再需要 `codexOnPollTick:` 参数（改成无参 —— 各 provider 的 `onPollTick` 由装配处单独设；见 SC4）；`backgroundIntervalSeconds` / 监听 `UserDefaults.didChangeNotification` 重起的逻辑不变。注：当 Claude 在 backoff（如 60min）而 `pollingMinutes`（如 30min）较短时，backoff 解析度被 round 到下一个 tick 边界（最坏多等 ~`pollingMinutes` —— backoff 本就是「别 hammer 服务器」，多等无害）—— 这是相对 v0.2.10「Claude 自持 timer 精确按 backoff 间隔」的预期行为变化。"
    done: false
    evidence: null
  - id: SC4
...
automated_checks:
  ...
  - "SC_AUTO_NO_TIMER: grep -n 'private var timer\\|scheduleTimer\\|func startPolling' macos/Sources/ClaudeUsageBar/UsageService.swift  →  无命中（自持 timer 已退役）"
manual_checks:
...
| Codex 菜单栏 glyph | 代码绘制的单色 `NSBezierPath` glyph（OpenAI 六瓣花结简化版 或 `</>`），不引入图片资源 | 没有 Codex logo PNG 可放；代码绘制 + template image 足够；真·资源可后续替换 |
...
popover 打开 → PopoverView.task（无 id）→ coordinator.refreshAllEnabledOnOpen()：for id in availableIDs { if nextEligibleRefresh 在未来则跳，否则 await refreshNow() }   // Claude 首屏空（snapshot==nil、不在 backoff）会被拉一次
...
2. **backoff 解析度变粗** —— 已在 §1/§2/SC3 写明、manual_checks 提了。可接受。
...
4. **`onPollTick` 协议成员要 `get set`** —— 协议没法给 `get set` 默认实现，所以每个 conformer（`UsageService`/`CodexProvider`/`StubProvider`）都得有 `var onPollTick: (@MainActor () -> Void)? = nil` 存储属性。`CodexProvider` 已有。实施时给另两个加。
