...
automated_checks:
  ...
  - "SC_AUTO_NO_RAW_LOG: grep -rn 'print(\\|NSLog\\|os_log' macos/Sources/ClaudeUsageBar/CodexRolloutCostParser.swift macos/Sources/ClaudeUsageBar/CodexUsageCollector.swift  →  无命中（不打印解析中内容/路径）"
manual_checks:
...
| `ClaudeUsageBarApp.swift` | `@StateObject private var codexStats = UsageStatsService(provider: .codex)`；`PopoverView(..., codexStats: codexStats)`；`.task` 里 `await codexStats.refresh()`（在 `await usageStats.refresh()` 后）；`if let codex = coordinator.provider(.codex) as? CodexProvider { codex.onPollTick = { Task.detached { await codexStats.refresh() } } }`（在 `codex.startPolling()` 前）。 |
...
| 🔧 | `ClaudeUsageBarApp.swift`（`codexStats` `@StateObject` + `.task` refresh + `onPollTick` 接线） |
