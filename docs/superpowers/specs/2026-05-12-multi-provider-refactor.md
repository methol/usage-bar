...
automated_checks:
  ...
  - "SC_AUTO_GREP_SC7: git grep -n 'UsageResponse\\|UsageBucket' macos/Sources/UsageBar -- ':!*UsageService.swift' ':!*UsageModel.swift'  # 期望仅剩文档注释（ProviderUsageSnapshot.swift / UsageHeroCard.swift 各一处）；UsageService 实施期决定不改名为 ClaudeUsageProvider"
manual_checks:
...
### 3.1 新增类型（`macos/Sources/UsageBar/Provider/` 新目录，或平铺在现有目录——实现期定，推荐建子目录）
...
现状：`UsageBarApp.task` 里做 history load、wire services、`bootstrapFromCLIIfNeeded`、首次 `usageStats.refresh()`、`service.startPolling()`。重构后：

- `UsageBarApp` `@StateObject` 改成持有 `ProviderCoordinator`（内部 new `ProviderRegistry` → new `ClaudeUsageProvider` + 它的 `ProviderRuntime`）、`UsageHistoryService`、`NotificationService`、`AppUpdater`、`UsageStatsService`。
...
| 🔧 | `UsageBarApp.swift` | `@StateObject` 改成 `ProviderCoordinator`；`.task` 装配迁移 |
