...
> ⚠️ **前置依赖**：本 spec 落地在 [v0.2.5 多供应商架构重构](./2026-05-12-multi-provider-refactor.md) **之后**。本文 §3「设计」是按"还没有多 provider 抽象"写的初稿，**待 v0.2.5 落地后需重写 §3**：到时 Codex 只需新增一个 `CodexProvider: UsageProvider`（`fetchSnapshot` 读 `~/.codex/auth.json` → `wham/usage` → 映射成 `ProviderUsageSnapshot`），视图/状态容器/tab 切换全部复用 v0.2.5 的泛化层，不再需要本文当前列的那些独立 `CodexUsageService`/`CodexUsageView` 文件。SC1-7 不变（验收目标一致），变的只是实现路径。
...
| 文件 | 职责 |
|---|---|
| `CodexCredentials.swift` | `struct CodexCredentials { accessToken; refreshToken?; idToken?; accountId? }` + `enum CodexCredentialStore { static func load(env:) throws -> CodexCredentials? }`：定位 `~/.codex/auth.json`（环境变量 `CODEX_HOME` 设了就用 `$CODEX_HOME/auth.json`），解析该 JSON —— 若**文件顶层**有 `OPENAI_API_KEY` 字段（注意：是 auth.json 里的 JSON key，不是进程环境变量）→ accessToken=该值、其余 nil；否则取 `tokens.{access_token\|accessToken, refresh_token\|refreshToken, id_token\|idToken, account_id\|accountId}`。文件不存在 → 返回 `nil`（静默）；存在但解析失败 → throw（error case 名不带 raw 值，SC7）。`internal`（非 private）让 `@testable import` 单测能直接 decode。 |
