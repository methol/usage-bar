...
spec_criteria:
  ...
  - id: SC7
    ...
    evidence: "(a) testCodexCredentialErrorDescriptionHasNoRawValues / testClientUnauthorized / testClientServerErrorOmitsBody（断言 `\"\\(error)\"` 不含 SENTINEL / SECRET_BODY）；测试 fixture 用哨兵值 ACCESS_SENTINEL 等而非像真 token 的串，断言失败 message 用 mask()。(b) grep -rnE 'accessToken|refreshToken|idToken|accountId' macos/Sources/UsageBar/Codex*.swift —— 命中仅：struct 字段声明、`Bearer \\(accessToken)` header value、`ChatGPT-Account-Id` header value、JSON key 字符串；无 print/os_log/description/error 文案插值。CodexUsageError/CodexCredentialError 的 description 只回 case 名（server 带 HTTP 状态码，非凭证）。"
automated_checks:
...
### 3.1 新增文件（全部 `macos/Sources/UsageBar/`）
...
| 单测 `macos/Tests/UsageBarTests/CodexProviderTests.swift` | 见 §3.4。 |
...
| `UsageBarApp.swift` | `ProviderCoordinator(claude: UsageService())` → `ProviderCoordinator(claude: UsageService(), additionalProviders: [CodexProvider()])`。其余装配不变（`MenuBarLabel(runtime: coordinator.primaryRuntime, …)` 仍只可能是 Claude 的 runtime）。 |
...
| 动作 | 文件 | 备注 |
|---|---|---|
| 🆕 | `macos/Sources/UsageBar/CodexCredentials.swift` | 只读 `~/.codex/auth.json`（尊重 `CODEX_HOME`） |
| 🆕 | `macos/Sources/UsageBar/CodexUsageModel.swift` | 线缆形状 + `normalizedWindows()` + `asProviderSnapshot()` |
| 🆕 | `macos/Sources/UsageBar/CodexUsageClient.swift` | `GET wham/usage`，错误不含凭证/ body |
| 🆕 | `macos/Sources/UsageBar/CodexProvider.swift` | `CodexProvider: UsageProvider`（`supportsBackgroundPolling = false`） |
| 🆕 | `macos/Tests/UsageBarTests/CodexProviderTests.swift` | §3.4 |
| 🔧 | `macos/Sources/UsageBar/ProviderUsageSnapshot.swift` | `CreditLine` 加 `remainingAmount?` / `isUnlimited` |
| 🔧 | `macos/Sources/UsageBar/ProviderUsageSection.swift` | `CreditLineRow` 补 unlimited / remaining 分支 |
| 🔧 | `macos/Sources/UsageBar/PopoverView.swift` | 抽 `ProviderUsageArea`（`@ObservedObject runtime`）—— 顺带清 v0.2.5 G5 nit ② |
| 🔧 | `macos/Sources/UsageBar/ProviderCoordinator.swift` | 加 `primaryEligibleIDs`；stored 校验收紧成 eligible |
| 🔧 | `macos/Sources/UsageBar/SettingsView.swift` | Primary Provider Picker 数据源 → `primaryEligibleIDs` |
| 🔧 | `macos/Sources/UsageBar/UsageBarApp.swift` | `additionalProviders: [CodexProvider()]` |
