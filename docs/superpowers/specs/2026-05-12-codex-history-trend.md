...
spec_criteria:
  - id: SC1
    ...
    evidence: "UsageHistoryService 加 `init(filename: String = \"history.json\", directory: URL? = nil)`，把原 `static historyFileURL` 换成实例 `let fileURL`/`backupURL`（`internal` 供测试）；`loadHistory`/`flushToDisk` 用 `fileURL`、坏文件挪到 `backupURL`；逻辑其余不变。UsageHistoryServiceTests 新建 4 用例：testInitDefaultPathUnchanged（断言 `fileURL` 末段 `history.json`、`backupURL` 末段 `history.bak.json`、父目录 `.config/claude-usage-bar`）/ testRecordFlushReloadCustomFile（临时目录 record→flush→新实例 load 回来值对）/ testTwoFilenamesNoCollision / testLoadCorruptFileMovesToBak。"
  - id: SC2
    ...
    evidence: "CodexProvider 加 `let history: UsageHistoryService`，`init(...history: UsageHistoryService? = nil)` —— 默认值在 @MainActor init 体内现造（`history ?? UsageHistoryService(filename: \"history-codex.json\")`，避免 @MainActor 默认参数在 nonisolated 上下文求值的编译错），init 末 `history.loadHistory()`；`refreshNow()` 成功分支 `runtime.setSuccess` 后调 `recordHistorySample(from:)`：`guard p != nil || s != nil`，`history.recordDataPoint(pct5h: (p ?? 0)/100, pct7d: (s ?? 0)/100)`。CodexProviderTests 新增：testRefreshSuccessRecordsHistorySample（两窗口 40%/60% → 1 点 0.40/0.60）/ testRefreshFreePlanRecordsZeroSession（单 604800 窗口 55% → primary 为 nil、1 点 pct5h=0/pct7d=0.55）/ testRefreshFailureRecordsNothing（401 → 0 点）/ testRefreshNoCredentialsRecordsNothing（无 auth.json → 0 点、snapshot nil）。"
  - id: SC3
  ...
  - id: SC6
    ...
    evidence: "`cd macos && swift build -c release` Build complete；`swift test` = 217 tests 0 failures（207 既有 + UsageHistoryServiceTests 4 + CodexProviderTests 6 新）；`make release-artifacts` 产出 zip/dmg；`bash macos/scripts/verify-release.sh macos/ClaudeUsageBar.zip` 与 `.dmg` 均 \"Release archive looks good\"。"
  - id: SC7
    ...
    evidence: "`UsageDataPoint` 字段（pct5h/pct7d JSON key）/ `history.json` 默认路径未动（testInitDefaultPathUnchanged）；Claude `claudeUsageArea` 的 `UsageChartSectionView(historyService:recentEvents:)` 调用走默认参数 → 仍 `5h`/`7d`；`MenuBarLabel`/`MenuBarIconRenderer`/`SettingsView` 文件未动；`supportsBackgroundPolling` 仍 false（testSupportsBackgroundPollingIsFalse + testCoordinatorPrimaryEligibleExcludesNonPollingProvider）；既有 217 tests 全绿（含 UsageServiceTests/ProviderAbstractionTests/UsageChartInterpolationTests/SettingsViewTests）；`ProviderCoordinator` 逻辑只改注释。"
automated_checks:
...
- [x] SC6 — swift build OK；swift test 217 tests 0 failures；make release-artifacts + verify zip/dmg OK
- [x] SC7 — Claude tab / 菜单栏 / Settings 零回归（Codex 不进 primary 下拉；既有 217 tests 绿）
