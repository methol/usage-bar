...
spec_criteria:
  ...
  - id: SC6
    ...
    evidence: "`cd macos && swift build -c release` Build complete；`swift test` = 219 tests 0 failures（207 既有 + UsageHistoryServiceTests 5 + CodexProviderTests 7 新）；`make release-artifacts` 产出 zip/dmg；`bash macos/scripts/verify-release.sh macos/ClaudeUsageBar.zip` 与 `.dmg` 均 \"Release archive looks good\"。"
  - id: SC7
